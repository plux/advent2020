...
part2(Ints) ->
    ...
    find_weakness(array:from_list(Ints), Valid, 0, 1).

find_weakness(Nums, Num, Start, Len) ->
    Set = [array:get(I, Nums) || I <- lists:seq(Start, Start+Len)],
    Sum = lists:sum(Set),
    if Sum == Num -> lists:min(Set) + lists:max(Set);
       Sum >  Num -> find_weakness(Nums, Num, Start+1, 1);
       Sum <  Num -> find_weakness(Nums, Num, Start, Len+1)
    end.
